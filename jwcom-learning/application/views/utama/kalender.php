...
$hari_ini = $seminggu[$hari];
  ...
  $kalender = "<table style=' padding:10px' cellspacing=1 cellpadding=3 
               border=0 class=tabel_data width=100%>\n";
  $kalender .= "<tr class=tr_terang>
               <td colspan=7 ><center><font face=tahoma size=5 color=#428bca> $hari_ini $tgl $bulanan[$bln] $thn
               </center></td></tr>\n";
  ...
  while ($adabaris) {
    ...
    for ($i = 0; $i < 7; $i++ ) {
      if ($mulaicetak < $haritglsatu) {
        $kalender .= "<td style='background:#428bca; border:1px solid #8a8a8a;'>&nbsp;</td>";
        $mulaicetak++;
      } 
      elseif ($a <= $jmlhari) {
        $tt = $a;
        if ($a == $tanggal) { 
          $tt = "<span  style='color: green; font-weight: bold; 
        ...
        $kalender .= "<td style='background:white; border:1px solid #8a8a8a;'>$tt</td>";
        $a++;
      } 
      else {
        $kalender .= "<td style='background:#428bca; border:1px solid #8a8a8a;'>&nbsp;</td>";
