...
class C_diskusi Extends CI_Controller{
	...
	public function tambah_aksi() //diskusi
		{	
			$data['varData']= $this->M_user->tampil_data(); //user login
			$tbl['mtr']= $this->M_diskusi->kategori_mtr();
			$this->form_validation->set_rules('id_materi', 'Id_materi', 'required',array('required' => 'KOLOM TIDAK BOLEH KOSONG!'));
			$this->form_validation->set_rules('isi', 'Isi', 'required|min_length[5]');
			$this->form_validation->set_rules('topik', 'Topik', 'required|min_length[5]');
            ...
            $_materi = htmlspecialchars($_POST['id_materi']);
        ...
        date_default_timezone_set('Asia/Jakarta');
     	$now = date('Y-m-d H:i:s');
			...
			$this->M_diskusi->inputdiskusi($insert);
